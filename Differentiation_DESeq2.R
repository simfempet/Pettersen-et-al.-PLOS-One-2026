...
Eset <- new("ExpressionSet", exprs = Counts_VST)



Eset.s <- standardise(eset = Eset)
...
min_vals <- Dmin(Eset.s, m = m, crange = 2:20, repeats = 3)



Dmin_df <- data.frame(Distance = min_vals, Clusters = c(2:20))



ggplot(Dmin_df, aes(x = Clusters, y = Distance))+
  geom_point(color = "black", fill = "steelblue", alpha = 0.7, pch = 21, size = 3)+
  geom_line(color = "steelblue")+
  geom_vline(xintercept = 7, linetype = "dashed")+
  labs(x = "Number of clusters", y = "Centroid distance")+
  theme_few()
...
mfuzz.plot(Eset.s, cl, mfrow = c(2,4))



Eset_expression <- assay(Eset.s)
...
Mfuzz_df <- cbind(Eset_expression, clusters)
...
colnames(membership) <- c("c1", "c2", "c3", "c4", "c5", "c6", "c7")



cluster_list <- lapply(1:7, function(k) {
  df_cluster <- Mfuzz_df[Mfuzz_df$clusters == k, ]
  mem_cluster <- membership[rownames(df_cluster), paste0("c", k), drop = FALSE]
  df_cluster$membership <- mem_cluster[, 1]
  return(df_cluster)
})



names(cluster_list) <- paste0("Mfuzz_c", 1:7)



Mfuzz_df$clusters <- as.numeric(Mfuzz_df$clusters)



Mfuzz_long <- Mfuzz_df %>%
  rownames_to_column("Gene") %>%
  pivot_longer(cols = colnames(Counts_VST),
               names_to = "Sample", 
               values_to = "Expression") %>%
  mutate(Membership = membership[cbind(Gene, paste0("c", clusters))])



Mfuzz_long$Sample <- factor(Mfuzz_long$Sample, levels = colnames(Counts_VST))



ggplot(filter(Mfuzz_long, Mfuzz_long$clusters %in% c(1, 3, 6, 7)), 
       aes(x = Sample, y = Expression, group = Gene, color = Membership, alpha = Membership)) +
  geom_line(linewidth = 0.1) +
  stat_summary(fun = mean, geom = "line", aes(group = 1), linewidth = 1.5)+
  scale_color_viridis_c(option = "rocket") +
  facet_wrap(~ clusters, ncol = 2) +  
  labs(x = "Sample", y = "Expression", color = "Membership") +
  guides(alpha = "none") +
  theme_few() +
...
clusters_to_subset <- c(1, 3, 6, 7)
subset_clusters <- list()
for (i in clusters_to_subset) {
  subset_clusters[[paste0("cluster", i)]] <- subset(cluster_list[[i]], cluster_list[[i]]$membership >= 0.7)
}
for (i in seq_along(subset_clusters)) {
  subset_clusters[[i]] <- rownames(subset_clusters[[i]])
}



GO_c1 <- enrichGO(gene = subset_clusters[[1]], OrgDb = "org.Hs.eg.db", ont = "BP", keyType = "SYMBOL", universe = rownames(Counts_VST))
GO_c1_df <- data.frame(GO_c1)



GO_c3 <- enrichGO(gene = subset_clusters[[2]], OrgDb = "org.Hs.eg.db", ont = "BP", keyType = "SYMBOL", universe = rownames(Counts_VST))
GO_c3_df <- data.frame(GO_c3)



GO_c6 <- enrichGO(gene = subset_clusters[[3]], OrgDb = "org.Hs.eg.db", ont = "BP", keyType = "SYMBOL", universe = rownames(Counts_VST))
GO_c6_df <- data.frame(GO_c6)



GO_c7 <- enrichGO(gene = subset_clusters[[4]], OrgDb = "org.Hs.eg.db", ont = "BP", keyType = "SYMBOL", universe = rownames(Counts_VST))
GO_c7_df <- data.frame(GO_c7)
...
GO_c6_df$Cluster <- "C6"
GO_c7_df$Cluster <- "C7"
Mfuzz_GO <- rbind(GO_c1_df, GO_c3_df, GO_c6_df, GO_c7_df)
...
#Save outputs
saveRDS(cl, file = "Skole/UiB/MCD-model/RNA-seq/Mfuzz_cluster.RDS")
write.csv(Mfuzz_GO, file = "Skole/UiB/MCD-model/RNA-seq/Mfuzz_enrichment.csv")
