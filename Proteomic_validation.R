...
Experimental_design <- data.frame(condition = c(rep("Ctrl", 3),
                                  ...
                                  label = gsub("^LFQ\\.intensity\\.", "", colnames(data[, c(114:116, 120:122, 129:131)])))
...
LFQ_matrix <- data_unique[, LFQ_columns[c(1:3, 7:9, 16:18)]] 
...
ggplot(score_df, aes(x = Condition, y = Score, fill = Condition))+
  geom_dotplot(binaxis='y', stackdir='center', pch=21, color="black", dotsize=1.2)+
